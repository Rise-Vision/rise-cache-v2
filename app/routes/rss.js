...
const RssRoute = function(app, db, logger) {
  ...
  controller.on("get-data-error", (err, key, res, next) => {
    logger.error("Could not get rss data", key);
    res.statusCode = 500;
    next(err);
  });
  ...
  controller.on("delete-data-error", (err, key, res, next) => {
    logger.error("Could not delete rss data", key);
    res.statusCode = 500;
    next(err);
  });
